...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  7 08:25:21 2022
...
def downsample_image(A):
    B = A[34:194]
    B = rgb2gray(B)
    B = np.array(Image.fromarray(B).resize(size=(IM_SIZE, IM_SIZE)))
    return B
...
class ReplayMemory:
    def __init__(self,size=MAX_EXPERIENCES,frame_height=IM_SIZE, frame_width=IM_SIZE,
         ...
         #states and new_state in minibatch
...
def init_filter(shape):
    w = np.random.rand(*shape) * np.sqrt(2.0/np.prod(shape[1:]))
...
class HiddenLayer():
    def __init__(self,M1,M2,f=T.nnet.relu):
        ...
        self.W = theano.shared(W_init.astype(np.float32))
        b_init = np.zeros(M2)
        self.b = theano.shared(b_init.astype(np.float32))
        self.params = [self.W,self.b]
        self.f = f
...
class DQN:
    def __init__(self, K, conv_layer_sizes, hidden_layer_sizes):
        ...
        conv_out_op = theano.function(inputs=[X], outputs=conv_out,
        ...
        print('flattened output size:',flattened_output_size)
...
def play_one(
    ...
    while not done:
        ...
        if total_t % TARGET_UPDATE_PERIOD == 0:
            ...
            print(f'Copied model parameters to target network, total_t = {total_t}, period = {TARGET_UPDATE_PERIOD}')
            
        #take action
        action = model.sample_action(state,epsilon_change)
    ...
    return total_t, episode_reward, (datetime.now() - t0), num_step_in_episode, total_time_training/num_step_in_episode,epsilon
...
if __name__ == '__main__':
    ...
    for i in range(num_episodes):
        total_t, episode_reward, duration, num_step_in_episode, time_per_step, epsilon = play_one(
                env,
                total_t,
                experience_replay_buffer,
                model,
        ...
        step_counts[i] = num_step_in_episode
        
        last_100_avg = episode_rewards[max(0,i-100):i+1].mean()
        last_100_avg_steps = step_counts[max(0,i-100):i+1].mean()
        print('Episode:',i,
              'Duration:',duration,
              'Num steps:',num_step_in_episode,
